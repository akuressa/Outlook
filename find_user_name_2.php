<?php
	...
	if(isset($_POST['person_name']))
	{
		...
		if($_POST['person_name']!='')
		{
			...
			$execute_person=array(':person_name'=>stripcleantohtml($_POST['person_name'])."%");
		}
		...
		if(!empty($find_user))
		{
			...
			{
				$html_data.="<div class='data_row' onclick='value_send_2(\"".$values['person_name']."\")'>".$values['person_name']."</div>";
			}
		}
		else
		{
			$html_data.="<div class='no_data_row' onclick='value_send_2()'>No data found</div>";
		}
		print_r($html_data);
		exit;
	}
