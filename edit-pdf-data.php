<?php
	...
	{
		$pdf_id=substr(base64_decode($_GET['pdf_id']), 0, -5);
		$find_pdf= find('first', PDF, '*', "WHERE id=:id", array(':id'=>$pdf_id));
		if(!empty($find_pdf))
		{
			//do nothing
		}
		else
		{
			...
			$_SESSION['SET_FLASH'] = 'Ungültige Rechnungsdaten id.';
	...
	{
		$_SESSION['SET_TYPE'] = 'error';
		$_SESSION['SET_FLASH'] = 'Rechnungsdaten id fehlt.';
		header('location:'.DOMAIN_NAME_PATH.'pdf-data-list.php'.(isset($_GET["page"]) && $_GET["page"]!="" ? "?page=".$_GET["page"] : ""));
		exit;
	}
	if(isset($_POST['edit_pdf']))
	{
		...
		{
			$pdf_id=substr(base64_decode($_GET['pdf_id']), 0, -5);
			/*$find_pdf_reference_no=find('first', PDF, 'id, reference_no', "WHERE reference_no=:reference_no AND id <> :id", array(':reference_no'=>stripcleantohtml($_POST['reference_no']), ':id'=>$pdf_id));
			...
			{*/
				...
				$value_set="reference_no=:reference_no, pdf_date=:pdf_date, no_of_pages=:no_of_pages, editor_name=:editor_name, client_name=:client_name, left_address=:left_address, mid_content=:mid_content, product_details=:product_details, payment_type=:payment_type, product_total=:product_total, tax_total=:tax_total, grand_total=:grand_total";
				$execute=array(
				...
				$update_pdf=update(PDF, $value_set, 'WHERE id=:id', $execute);
				if($update_pdf == true)
				{
					$find_pdf= find('first', PDF, '*', "WHERE id=:id", array(':id'=>$pdf_id));
...
<?php
					...
					//$mpdf=new \Mpdf\Mpdf(array('','', 0, '', 5, 5, 16, 16, 10, 10, ''));//mpdf 7
					...
					$_SESSION['SET_FLASH'] = 'Rechnungsdaten erfolgreich aktualisiert.';
					header('location:'.DOMAIN_NAME_PATH.'pdf-data-list.php'.(isset($_GET["page"]) && $_GET["page"]!="" ? "?page=".$_GET["page"] : ""));
					exit;
				}
				else
...
<head>
	...
	<!--
		$(function(){
			$("#edit_pdf_form").validationEngine();
		...
		<div class="col-md-8">
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title">Rechnungsdaten bearbeiten</h3>
				</div>
				<div class=" content">
					<div class="col-md-3">&nbsp;</div>
					<div class="col-md-6">
						<form id="edit_pdf_form" name="edit_pdf_form" method="post" enctype="multipart/form-data" action="">
							...
							<div class="form-group" style="padding:6px;position:relative;">
							  ...
							  <input class="form-control validate[required]" placeholder="Kundenname" type="text" id="client_name" name="client_name" maxlength="255"  data-errormessage-value-missing="Kundenname ist erforderlich" value="<?php echo((isset($_POST['client_name']) && $_POST['client_name']!='') ? $_POST['client_name'] : (isset($find_pdf['client_name']) && $find_pdf['client_name']!="" ? $find_pdf['client_name'] : ""));?>" autocomplete="off"/>
							  <div class="auto_sugg_cls" id="auto_sug_content_div"></div>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Adresse:</label>
							  <textarea name="left_address" class="notes_cls form-control validate[required]" Placeholder="Zahlungsdetails" data-errormessage-value-missing="Adresse ist erforderlich" id="left_address" tabindex="1"><?php echo(isset($_POST['left_address']) && $_POST['left_address']!="" ? $_POST['left_address'] : (isset($find_pdf['left_address']) && $find_pdf['left_address']!="" ? $find_pdf['left_address'] : ""));?></textarea>
							</div>
							<div class="form-group" style="padding:6px;display:none;">
							  <label><font color = "red">*</font> Rechnung Nr.:</label>
							  <input class="form-control validate[required]" placeholder="Rechnung Nr." type="text" id="reference_no" name="reference_no" maxlength="255"  data-errormessage-value-missing="Rechnung Nr. erforderlich" tabindex="2" value="<?php echo((isset($_POST['reference_no']) && $_POST['reference_no']!='') ? $_POST['reference_no'] : (isset($find_pdf['reference_no']) && $find_pdf['reference_no']!="" ? $find_pdf['reference_no'] : ""));?>"/>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Datum:</label>
							  <input class="form-control validate[required]" placeholder="Datum" type="text" id="pdf_date" name="pdf_date" maxlength="10"  data-errormessage-value-missing="Datum erforderlich" tabindex="3" value="<?php echo((isset($_POST['pdf_date']) && $_POST['pdf_date']!='') ? $_POST['pdf_date'] : (isset($find_pdf['pdf_date']) && $find_pdf['pdf_date']!="" ? change_date_format($find_pdf['pdf_date']) : date("d.m.Y")));?>" readonly/>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Seite:</label>
							  <input class="form-control validate[required, custom[integer]]" placeholder="Seite" type="text" id="no_of_pages" name="no_of_pages" maxlength="11"  data-errormessage-value-missing="Seite erforderlich" tabindex="4" value="<?php echo((isset($_POST['no_of_pages']) && $_POST['no_of_pages']!='') ? $_POST['no_of_pages'] : (isset($find_pdf['no_of_pages']) && $find_pdf['no_of_pages']!="" ? $find_pdf['no_of_pages'] : ""));?>"/>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Herausgeber:</label>
							  <input class="form-control validate[required]" placeholder="Herausgeber" type="text" id="editor_name" name="editor_name" maxlength="255"  data-errormessage-value-missing="Herausgeber erforderlich" tabindex="5" value="<?php echo((isset($_POST['editor_name']) && $_POST['editor_name']!='') ? $_POST['editor_name'] : (isset($find_pdf['editor_name']) && $find_pdf['editor_name']!="" ? $find_pdf['editor_name'] : ""));?>"/>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Inhalt:</label>
							  <textarea type="text" name="mid_content" id="mid_content" class="ckeditor" style="width:100%;padding:10px;height: 100px;" tabindex="6"><?php echo((isset($_POST['mid_content']) && $_POST['mid_content']!='') ? $_POST['mid_content'] : (isset($find_pdf['mid_content']) && $find_pdf['mid_content']!="" ? $find_pdf['mid_content'] : ""));?></textarea>
							</div>
							<div class="form-group" style="padding:6px">
							  <label><font color = "red">*</font> Produkt:</label>
							<?php
								if(isset($find_pdf['product_details']) && $find_pdf['product_details']!="")
								{
									$added_product_array=json_decode($find_pdf['product_details'], true);
								}
							...
							?>
							  <div id="" class="">
									<div class="col-md-4" style="padding-top: 10px;">
										<input type="checkbox" id="product_<?php echo $product_list_val['id'];?>" name="product_details[<?php echo $product_list_val['id'];?>]" value="<?php echo $product_list_val['id'];?>" <?php echo(isset($_POST['product_details']) && in_array($product_list_val['id'], $_POST['product_details']) ? "checked='checked'" : (isset($find_pdf['product_details']) && $find_pdf['product_details']!="" && isset($added_product_array) && isset($added_product_array[$product_list_val['id']]) ? "checked='checked'" : ""));?> style="margin-top:0px;" class="product_checkbox validate[groupRequired[product]]" data-price="<?php echo(number_format($product_list_val['price'],2));?>" data-tax_type="<?php echo $product_list_val['tax_type'];?>" data-tax="<?php echo $product_list_val['tax'];?>">&nbsp;&nbsp;<em><?php echo $product_list_val['item_no'].". ".$product_list_val['item_name'];?></em>
									</div>
									<div class="col-md-3" style="padding-top: 10px;">€&nbsp;<?php echo(number_format($product_list_val['price'],2));?></div>
									<div class="col-md-5">
										Anzahl: <input type="text" id="qty_<?php echo $product_list_val['id'];?>" name="product_qty[<?php echo $product_list_val['id'];?>]" value="<?php echo(isset($_POST['product_qty']) && isset($_POST['product_qty'][$product_list_val['id']]) && $_POST['product_qty'][$product_list_val['id']]!="" ? $_POST['product_qty'][$product_list_val['id']] : (isset($find_pdf['product_details']) && $find_pdf['product_details']!="" && isset($added_product_array) && isset($added_product_array[$product_list_val['id']]) ? $added_product_array[$product_list_val['id']]['qty'] : 1));?>" class="form-control" style="width:60px;display: inline-block;" onchange="product_qty_fun($(this))">
									</div>
							...
							<div class="form-group" style="padding:6px">
								<label>
									Gesamtsumme netto: 
									<span id="product_total"><?php echo((isset($_POST['hidden_product_total']) && $_POST['hidden_product_total']!='') ? "&euro;".$_POST['hidden_product_total'] : (isset($find_pdf['product_total']) && $find_pdf['product_total']!="" ? "&euro;".$find_pdf['product_total'] : ""));?></span>
									<input type="hidden" name="hidden_product_total" id="hidden_product_total" value="<?php echo((isset($_POST['hidden_product_total']) && $_POST['hidden_product_total']!='') ? $_POST['hidden_product_total'] : (isset($find_pdf['product_total']) && $find_pdf['product_total']!="" ? $find_pdf['product_total'] : ""));?>">
								</label><br/>
								<label>
									Mwst: 
									<span id="tax_total"><?php echo((isset($_POST['hidden_tax_total']) && $_POST['hidden_tax_total']!='') ? "&euro;".$_POST['hidden_tax_total'] : (isset($find_pdf['tax_total']) && $find_pdf['tax_total']!="" ? "&euro;".$find_pdf['tax_total'] : ""));?></span>
									<input type="hidden" name="hidden_tax_total" id="hidden_tax_total" value="<?php echo((isset($_POST['hidden_tax_total']) && $_POST['hidden_tax_total']!='') ? $_POST['hidden_tax_total'] : (isset($find_pdf['tax_total']) && $find_pdf['tax_total']!="" ? $find_pdf['tax_total'] : ""));?>">
								</label><br/>
								<label>
									Gesamtsumme brutto: 
									<span id="grand_total"><?php echo((isset($_POST['hidden_grand_total']) && $_POST['hidden_grand_total']!='') ? "&euro;".$_POST['hidden_grand_total'] : (isset($find_pdf['grand_total']) && $find_pdf['grand_total']!="" ? "&euro;".$find_pdf['grand_total'] : ""));?></span>
									<input type="hidden" name="hidden_grand_total" id="hidden_grand_total" value="<?php echo((isset($_POST['hidden_grand_total']) && $_POST['hidden_grand_total']!='') ? $_POST['hidden_grand_total'] : (isset($find_pdf['grand_total']) && $find_pdf['grand_total']!="" ? $find_pdf['grand_total'] : ""));?>">
								</label><br/>
							</div>
							<div class="form-group" style="padding:6px">
								<label><font color = "red">*</font> Paymenttyp:</label><br/>
								<?php
									if(isset($find_pdf['payment_type']) && $find_pdf['payment_type']!='')
									{
										$stored_payment_type=explode(", ", $find_pdf['payment_type']);
									}
									foreach($payment_type_array as $type_key=>$type_val)
									{
								?>
								<input type="checkbox" id="payment_type_<?php echo $type_key;?>" name="payment_type[]" value="<?php echo $type_val;?>" <?php echo(isset($_POST['payment_type']) && in_array($type_val, $_POST['payment_type']) ? "checked='checked'" : (isset($find_pdf['payment_type']) && $find_pdf['payment_type']!="" && isset($stored_payment_type) && in_array($type_val, $stored_payment_type) ? "checked='checked'" : ""));?> style="margin-top:0px;" class="validate[groupRequired[paymenttype]]">&nbsp;&nbsp;<em><?php echo $type_val;?></em>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<?php
									}
								?>
							</div>
							<div class="form-group" style="padding:3px;text-align:right;">
							  <button class="button" name = "edit_pdf" tabindex="5"><b>Rechnungsdaten bearbeiten</b></button>
							</div>
						</form>
					</div>
					<div class="col-md-3">&nbsp;</div>
