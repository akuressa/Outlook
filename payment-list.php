			  ...
			  <div class=" content">
				...
				<?php
					if(!empty($payment_list))
					{
						...
						{
							$total_due=$payment_val['amount_due'];
							$find_extra_payment= find('all', EXTRA_PAYMENT, '*', "WHERE payment_info_master_id=:payment_info_master_id", array(':payment_info_master_id'=>$payment_val['id']));
							if(isset($find_extra_payment) && !empty($find_extra_payment))
							{
								foreach($find_extra_payment as $pay_key=>$pay_val)
								{
									if($pay_val['payment_type']=="N")
									{
										$total_due=$total_due-$pay_val['amount'];
									}
									else if($pay_val['payment_type']=="P")
									{
										$total_due=$total_due+$pay_val['amount'];
									}
								}
				...
				?>
						 <tr>
							...
							<td><?php echo ($total_due ? "&euro;".number_format($total_due, 2, ".", ",") : "");?></td>
