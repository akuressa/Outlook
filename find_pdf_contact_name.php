<?php
	...
	if(isset($_POST['client_name']))
	{
		...
		$find_user= find('all', USERS, 'id, status, person_name, address', $where_clause." LIMIT 0, 50", $execute);
		$html_data="";
		if(!empty($find_user))
		{
			foreach($find_user as $key=>$values)
			{
				$html_data.="<div class='data_row' onclick='value_send(\"".$values['person_name']."\", \"".strip_tags(preg_replace('/\s\s+/', '@@', nl2br($values['address'])))."\")'>".$values['person_name']."</div>";
			}
		}
		else
		{
