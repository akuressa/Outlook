<?php
	...
	if(isset($_POST['edit_contact']))
	{
		...
		{
			$_SESSION['SET_TYPE'] = 'error';
			$_SESSION['SET_FLASH'] = 'Anrede ist erforderlich.';
		}
		else if(isset($_POST['person_name']) && $_POST['person_name']=='')
		{
			$_SESSION['SET_TYPE'] = 'error';
			$_SESSION['SET_FLASH'] = 'Vollst&auml;ndiger name ist erforderlich.';
		}
		else if(isset($_POST['date_of_birth']) && $_POST['date_of_birth']=='')
		{
			$_SESSION['SET_TYPE'] = 'error';
			$_SESSION['SET_FLASH'] = 'Geburtsdatum ist erforderlich.';
		}
		...
		else
		{
			...
			else
			{
				...
				{
					$_SESSION['SET_TYPE'] = 'error';
					$_SESSION['SET_FLASH'] = 'Kundenname bereits vorhanden ist.';
				}
				else
				{
					$_SESSION['SET_TYPE'] = 'error';
	...
	if(isset($_POST['btn_create_notice'])) { 
		...
		{
			$_SESSION['SET_TYPE'] = 'error';
			$_SESSION['SET_FLASH'] = 'Hinweis ist erforderlich.';
		}
		else
		{
			$user_id=substr(base64_decode($_GET['contact_id']), 0, -5);
	...
	if(isset($_POST['btn_create_medical_log'])) { 
		...
		{
			$_SESSION['SET_TYPE'] = 'error';
			$_SESSION['SET_FLASH'] = 'Behandlungsprotokoll ist erforderlich.';
		}
		else
		{
			$user_id=substr(base64_decode($_GET['contact_id']), 0, -5);
			...
			{
				$_SESSION['SET_TYPE'] = 'success';
				$_SESSION['SET_FLASH'] = 'Behandlungsprotokoll erfolgreich hinzugef&uuml;gt.';
				header('location:'.DOMAIN_NAME_PATH.'edit_contact.php?contact_id='.$_GET['contact_id'].(isset($_GET["page"]) && $_GET["page"]!="" ? "&page=".$_GET["page"] : "").(isset($_GET["top_page"]) && $_GET["top_page"]!="" ? "&top_page=".$_GET["top_page"] : "").(isset($_GET["bottom_page"]) && $_GET["bottom_page"]!="" ? "&bottom_page=".$_GET["bottom_page"] : ""));
				exit;
			}
			else
	...
	if(isset($_POST['edit_user_medical_log']))
	{
		...
		else
		{
			...
			{
				$_SESSION['SET_TYPE'] = 'success';
				$_SESSION['SET_FLASH'] = 'Behandlungsprotokoll erfolgreich aktualisiert.';
				header('location:'.DOMAIN_NAME_PATH.'edit_contact.php?contact_id='.$_GET['contact_id'].(isset($_GET["page"]) && $_GET["page"]!="" ? "&page=".$_GET["page"] : "").(isset($_GET["top_page"]) && $_GET["top_page"]!="" ? "&top_page=".$_GET["top_page"] : "").(isset($_GET["bottom_page"]) && $_GET["bottom_page"]!="" ? "&bottom_page=".$_GET["bottom_page"] : ""));
				exit;
			}
			else
	...
	if(isset($_GET['medical_remove_index']))
	{
		...
		{
			$_SESSION['SET_TYPE'] = 'success';
			$_SESSION['SET_FLASH'] = 'Behandlungsprotokoll erfolgreich gel&ouml;scht.';
		}
		else
		{
			$_SESSION['SET_TYPE'] = 'error';
	...
	if (isset($_POST['btn_create_call_back'])) { 
		...
		else
		{
			...
			$add_call_note=save(CALL_BACK, "user_id, added_by, call_date, next_call_date, description", ":user_id, :added_by, :call_date, :next_call_date, :description", array(':user_id'=>$user_id, ':added_by'=>$_SESSION['logged_user_id'], ':call_date'=>$call_date, ':next_call_date'=>$next_call_date, ':description'=>$_POST['call_description']));
			if($add_call_note > 0)
			{
				$_SESSION['SET_TYPE'] = 'success';
	...
	if(isset($_POST['btn_edit_call_back']))
	{
		...
		else
		{
			...
			$update_call_note=update(CALL_BACK, "call_date=:call_date, next_call_date=:next_call_date, description=:description", "WHERE id=:id", array(':id'=>$_POST['call_back_array_key'], ':call_date'=>$call_date, ':next_call_date'=>$next_call_date, ':description'=>$_POST['edit_call_description']));
		...
		<div class="col-md-8">
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title">Kontakt bearbeiten</h3>
				</div>
				<div class=" content">
					<div class="col-md-4">
						<div class="left_notice_content">
							<div class="form-group" style="padding:15px 1px;">
							  ...
							  <a href = "javascript:void(0)" onClick="$('#edit_medical_main_div').hide();$('#medical_main_div').slideToggle('slow');" style="float:right;font-size:12px;margin: 5px;"><button class="button" name = "btn_create"><b>Behandlungsprotokoll Hinzuf&uuml;gen</b></button></a>
							...
							<table id="myTable" class="table tablesorter">
								...
								<tbody> 
									...
									?>
											<tr>
												<td colspan="<?php echo($admin_privilege==true ? "3" : "2");?>" class="no_record_cls">Kein Eintrag gefunden</td>
											</tr>
									<?php
										}
									?>
						...
						<div class="left_medical_log_content" style="margin-top:10px;">
							<div class="panel panel-primary">
								<div class="panel-heading">
									<h3 class="panel-title">Behandlungsprotokoll</h3>
								</div>
							</div>
							...
							<div class="message_main_div" id="medical_main_div">
								<div class="col-md-12">
									<form id="add_medical_log" name="add_medical_log" method="post" action="">
										...
										<div class="form-group" style="padding:6px">
										  <label><font color = "red">*</font> Behandlungsprotokoll:</label>
										  <textarea name="medical_log" class="form-control validate[required]" Placeholder="Behandlungsprotokoll" data-errormessage-value-missing="Behandlungsprotokoll ist erforderlich" id="medical_log"><?php echo(isset($_POST['medical_log']) && $_POST['medical_log']!="" ? $_POST['medical_log'] : "");?></textarea>
										</div>
										<div class="form-group" style="padding:3px;text-align:right;">
										  <button class="button" name = "btn_create_medical_log"><b>F&uuml;gen Behandlungsprotokoll</b></button>
										</div>
									</form>
								</div>
							</div>
							...
							<div class="message_main_div" id="edit_medical_main_div">
								<div class="col-md-12">
									<form id="edit_medical_log_form" name="edit_medical_log_form" method="post" action="">
										...
										<div class="form-group" style="padding:6px">
										  <label><font color = "red">*</font> Behandlungsprotokoll:</label>
										  <textarea class="form-control validate[required]" Placeholder="Behandlungsprotokoll" id="edit_medical_log" name="edit_medical_log" data-errormessage-value-missing="Behandlungsprotokoll ist erforderlich"><?php echo(isset($_POST['edit_medical_log']) && $_POST['edit_medical_log']!="" ? $_POST['edit_medical_log'] : "");?></textarea>
										...
										?>
										 <tr>
											<td>
												...
												<strong>Nächstes Rückrufdatum:</strong> <?php echo $next_call_date_time;?><br/>
							...
							<div class="message_main_div" id="call_back_message_main_div">
								<div class="col-md-12">
									<form id="add_call_back_message_form" name="add_call_back_message_form" method="post" action="">
										...
										<div class="form-group" style="padding:6px">
										  <label> Nächstes Rückrufdatum:</label>
					...
					<div class="col-md-8">
						<form id="edit_user" name="edit_user" method="post" enctype="multipart/form-data" action="">
							...
							<div class="form-group" style="padding:6px">
								...
								<div class="col-md-8">
									<input class="form-control validate[custom[email]]" placeholder="E-Mail-Addresse" type="text" name="email_address" id="email_address" value="<?php echo((isset($_POST['email_address']) && $_POST['email_address']!='') ? $_POST['email_address'] : (isset($find_user['email_address']) && $find_user['email_address']!="" ? $find_user['email_address'] : ""));?>" data-errormessage-value-missing="E-Mail ist erforderlich" maxlength="255" tabindex="6"/>
								</div>
								<div class="clearfix"></div>
							</div>
							<div class="form-group" style="padding:6px;display:none;">
							...
							<div class="form-group" style="padding:6px;">
								<div class="col-md-4">
								  ...
								  <br/>
								  <a href="javascript:void(0)" onclick="addNewDoc();">Füge ein anderes Dokument hinzu</a>
								  <br/>
								  <label></label>
								</div>
							...
							<div class="form-group" style="padding:3px;text-align:right;">
								...
								<button class="button" type="submit" name = "edit_contact" tabindex="13"><b>Anderung speichern</b></button>
							</div>
						</form>
					</div>
				</div>
